...
for (const path of (await Promise.all(checkedRoots.map(filesUnder))).flat()) {
  ...
  for (const [index, line] of lines.entries()) {
    for (const [pattern, message] of forbidden) {
      if (pattern.test(line)) violations.push(`${relative(".", path)}:${index + 1}: ${message}`);
      pattern.lastIndex = 0;
    }
  }
}
...
if (violations.length > 0) {
  console.error("Repository anti-pattern policy violations:");
  for (const violation of violations) console.error(`  ${violation}`);
  process.exitCode = 1;
} else {
  console.log("Repository anti-pattern policy: clean");
}
